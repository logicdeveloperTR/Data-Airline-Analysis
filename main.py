...
class Model:
    ...
    def do_algorithm_middle(self):
        ...
        if self.isFirst:
            for x in range(len(self.x[0])-1):
                arr=np.array([a[x+1] for a in self.x])
                for b in range(len(self.x)):
                    self.x[b][x+1]=(self.x[b][x+1]-np.sum(arr)/len(arr))/(np.amax(arr)-np.amin(arr))
            self.isFirst=False
